...
test('开仓收取手续费并冻结保证金（默认初始$10000，杠杆下限10x）', () => {
  const w = mkWallet();
  const pos = w.openPosition({ symbol: 'BTCUSDT', side: 'long', price: 100, margin: 500, leverage: 10, time: T0, strategy: 'short' });
  assert.ok(pos.id);
  const fee = 500 * 10 * FEE_RATE;
