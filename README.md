...
23. **模拟交易钱包**（钱包Tab）：$1000合约（单笔保证金$100-200、杠杆2-10x按策略分层、taker费0.05%/边、95%强平线、止损止盈自动执行、最多3仓+6h冷却）+ $1000 PM（每期唯一决策$50-100按置信度分档）；每小时权益快照、**每日收益复盘表**、每小时收益明细、开平仓与注单全历史；离线模拟行情下自动暂停以保证绩效真实。
