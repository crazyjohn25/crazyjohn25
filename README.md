...
21. **三层策略建议**（取代15m/30m碎片化策略）：短线1-6小时（1h主周期+4h过滤）/ 中短线6-24小时（4h+1d过滤）/ 长线1-3天（1d），大周期反向时信号自动降级（"不与大趋势作对"），融合**新闻面24h偏向**（短线权重最大）与**历史复盘经验**（低命中率策略自动降权），重大共振信号（|评分|≥3）**存档+提示音+模拟开仓**，每个策略附完整交易计划（入场/止损/目标/盈亏比/建议杠杆），盈亏比<1自动回退ATR目标。
...
- **模拟钱包v3**：初始资金可设置（默认$10000）、注资/出金、**手动平仓按钮**、开仓时间显示、币安标准费率（taker 0.05%/边 + **资金费0.01%/8h**）、强平、**只跟强烈信号开仓（最高30x）**、每日收益复盘只统计有真实平仓的日期
